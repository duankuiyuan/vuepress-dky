...
![](./imgs/2019-12-02-10-57-23.png)

**nodejs**

nodejs直接运行某个js文件，该文件被称之为入口文件

![](./imgs/2019-12-02-10-59-43.png)
...
任何一个正常的模块化标准，都应该默认隐藏模块中的所有实现，而通过一些语法和api调用来暴露接口

**暴露接口的过程即模块的导出**

![](./imgs/2019-12-02-11-27-12.png)

##模块的导入

当需要使用一个模块时候，使用的是该模块暴露的部分（导出的部分），隐藏的部分永远是无法使用的。
...
2. 模块内的所有代码均为隐藏代码，包括全局变量、全局函数，这些全局的内容均不应该对全局变造成任何污染
3. 如果一个模块需要暴露一些API提供给外部使用，需要通过exports导出，exports是一个空对象，你可以为对象添加任何需要导出的内容
4. 如果一个模块需要导入其他模块，通过require实现，require是一个函数，传入模块的路径即可返回该模块导出的整个内容

### nodejs对CommonJS的实现
为了实现CommonJS，nodejs对模块做出了以下处理

1. 为了保证高效的执行，仅加载必要的模块。nodejs只执行到require函数时才会加载并执行模块
...
3. 为了保证顺利的导出模块内容，nodejs做了以下处理
   ...
   3. 为了方便开发者便捷的导出，nodejs在初始化完```module.exports```后，又声明了一个变量```exports = module.exports```
    ...
    ```
    >所以在模块化代码中直接给 module.exports 赋值 和直接给 exports赋值结果会不一样，如果直接给exports赋值（exports = xxx）由于导出的是module.exports，所以导出的内容是取不到值的。
